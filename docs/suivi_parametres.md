...
- `[ ]` 7.1 — Page `/parametres/finance/page.tsx`
- `[ ]` 7.2 — Sélecteur de devise (GNF, EUR, USD, XOF)
- `[ ]` 7.3 — Liste configurable des modes de paiement
- `[ ]` 7.4 — Gestion des types de frais (CRUD)
- `[ ]` 7.5 — Fréquence de paiement (Mensuel / Trimestriel / Annuel)
- `[ ]` 7.6 — Pénalités de retard (% ou montant fixe + délai)
- `[ ]` 7.7 — Règles de réduction automatique (2ème enfant, 3ème enfant)
- `[ ]` 7.8 — Format de numérotation des reçus (préfixe configurable)
- `[ ]` 7.9 — PIN d'accès comptabilité (modifier le code existant)
...
- `[ ]` 6.1 — Page `/parametres/documents/page.tsx`
- `[ ]` 6.2 — Galerie de modèles de bulletins (Classique, Moderne, Officiel Guinéen, Minimaliste)
- `[ ]` 6.3 — Personnalisation de l'en-tête (Logo + Nom + Slogan + "République de Guinée")
- `[ ]` 6.4 — Toggle des champs du bulletin (Rang, Moyenne classe, Min/Max, Graphique, Photo)
- `[ ]` 6.5 — Configuration des appréciations automatiques
- `[ ]` 6.6 — Placement des zones de signature (Directeur, Prof Principal, Parent)
- `[ ]` 6.7 — Template de certificat de scolarité
- `[ ]` 6.8 — Template de reçu de paiement
- `[ ]` 6.9 — Filigrane / Watermark sur les PDF officiels

---

## 🔒 SECTION 8 : Sécurité & Gestion des Accès _(Priorité 8)_

- `[ ]` 8.1 — Page `/parametres/securite/page.tsx`
- `[ ]` 8.2 — Créer table `ss_roles` et `ss_permissions`
- `[ ]` 8.3 — Interface de création de rôles personnalisés
- `[ ]` 8.4 — Matrice de permissions par module (grille cochable)
- `[ ]` 8.5 — Politique de mot de passe (longueur, complexité, renouvellement)
- `[ ]` 8.6 — Durée de session (déconnexion automatique)
- `[ ]` 8.7 — Journal d'audit (table `ss_audit_log` + page de consultation)
...
| Section 7 — Finance | 9 | 0 | 0% |
| Section 2 — Calendrier | 5 | 0 | 0% |
| Section 6 — Documents | 9 | 0 | 0% |
| Section 8 — Sécurité | 7 | 0 | 0% |
