...
function GestionPaiements() {
    ...
    const filteredPaiements = paiements.filter(p => {
        ...
        const matchTerm = !term || p.numero_recu?.toLowerCase().includes(term) || p.eleve_nom?.toLowerCase().includes(term) || p.eleve_prenom?.toLowerCase().includes(term) || p.numero_facture?.toLowerCase().includes(term);
        
        let matchDebut = true;
        let matchFin = true;
        if (p.date_paiement) {
            const pDate = p.date_paiement.split('T')[0];
            if (encaisDateDebut) matchDebut = pDate >= encaisDateDebut;
            if (encaisDateFin) matchFin = pDate <= encaisDateFin;
        }
        
        return matchTerm && matchDebut && matchFin;
    });
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
                        ...
                        <div style={cardStyle}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                {groupedPaiements.slice(0, 5).map(group => {
